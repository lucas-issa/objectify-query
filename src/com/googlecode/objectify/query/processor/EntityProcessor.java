...
public class EntityProcessor extends AbstractProcessor {
	...
	private void processEntity(Element entityElement) {
		...
		try {
			out =
					new PrintWriter(new BufferedWriter(this.env.getFiler()
						.createSourceFile(queryQName, entityElement)
						.openWriter()));
			...
			for (VariableElement fieldElement : ElementFilter.fieldsIn(env.getElementUtils()
				.getAllMembers((TypeElement) entityElement))) {

				Unindexed unindexedField =
						fieldElement.getAnnotation(Unindexed.class);
				// Transient transientField =
				// fieldElement.getAnnotation(Transient.class);
				NotSaved notSavedField =
						fieldElement.getAnnotation(NotSaved.class);
				Indexed indexedField =
						fieldElement.getAnnotation(Indexed.class);
				...
				if (unindexedField != null
						&& unindexedField.value().length == 0) {
			...
			out.println("  @Override");
			out.println("  public QueryResultIterable<" + entityName
					+ "> fetch() {");
			...
			out.println("    ArrayList<" + entityName
			...
			if (parentField != null) {
				...
				fieldType =
						fieldType.substring(fieldType.indexOf('<') + 1,
				...
				out.println("    }");
				out.println();
				out.println("    Map<Long, " + fieldType
						+ "> list = ofy().get(" + fieldType
						+ ".class, idList);");
				out.println("    return new ListPage<" + fieldType
				...
				out.println("    ArrayList<Key<" + fieldType
						...
						+ ">>();");
				out.println("    for (int i = 0; i < pageSize && (more = iterator.hasNext()); i++) {");
				out.println("      idList.add(new Key<" + fieldType + ">("
						+ fieldType
						+ ".class, iterator.next().getParent().getId()));");
				out.println("    }");
				out.println();
				out.println("    return new ListPage<Key<"
						+ fieldType
			...
			for (ExecutableElement methodElement : ElementFilter.methodsIn(env.getElementUtils()
				.getAllMembers((TypeElement) entityElement))) {
				...
				if (methodElement.getParameters().size() == 0) {
					...
					if (listField != null) {
						...
						out.println("  public ListPage<Key<" + returnType
								+ ">> list" + listField.singularName()
						...
						out.println("  }");
						out.println();
						out.println("  public ListPage<" + returnType
								+ "> list" + listField.pluralName()
						...
						out.println();
						out.println("    boolean more = false;");
						out.println("    ArrayList<" + idType
								+ "> idList = new ArrayList<" + idType + ">();");
						out.println("    for (int i = 0; i < pageSize && (more = iterator.hasNext()); i++) {");
						out.println("      idList.add(iterator.next()."
								+ methodElement.getSimpleName().toString()
								+ "()." + idAccessor + ");");
						out.println("    }");
						out.println();
						out.println("    Map<" + idType + ", " + returnType
								+ "> list =");
						out.println("      ofy().get(" + returnType
								+ ".class, idList);");
			...
			throw new RuntimeException(
					"Query already exists or cannot be created");
		} finally {
			if (out != null) {
				out.close();
			}
